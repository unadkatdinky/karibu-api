...
func Login(c *gin.Context) {
	...
	// ---- STEP 4: Block login for unverified accounts ----
	if !isVerified {
		c.JSON(http.StatusForbidden, gin.H{
			"error": "Please verify your email before logging in",
			"email": user.Email,
		})
		log.Printf("⚠️ Login blocked for unverified account: %s", user.Email)
		return
	}
...
func Logout(c *gin.Context) {
	...
	setAuthCookie(c, "karibu_access", "", -1)
	setAuthCookie(c, "karibu_refresh", "", -1)
...
func generateAndSetTokens(c *gin.Context, userID string, role string) {
	...
	// main.go already refuses to start if these are unset
	...
	// ---- Set Access Token as HTTP-Only Cookie ----
	setAuthCookie(c, "karibu_access", accessTokenString, 15*60)

	// ---- Set Refresh Token as HTTP-Only Cookie ----
	setAuthCookie(c, "karibu_refresh", refreshTokenString, 7*24*60*60)
